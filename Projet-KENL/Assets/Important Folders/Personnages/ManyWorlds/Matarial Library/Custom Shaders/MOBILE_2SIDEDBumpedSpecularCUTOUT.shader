...
/////// Normals:
                ...
                float3 normalLocal = lerp(UnpackNormal(tex2D(_Normal,TRANSFORM_TEX(node_441.rg, _Normal))).rgb,float3(0,0,1),_NormalBurn);
                ...
                clip(tex2D(_CutoutMask,TRANSFORM_TEX(node_441.rg, _CutoutMask)).a - 0.5);
...
///////// Gloss:
                float gloss = lerp( _Shininess, (tex2D(_Gloss,TRANSFORM_TEX(node_441.rg, _Gloss)).a*_Shininess), _UseGlossMap );
                float specPow = exp2( gloss * 10.0+1.0);
////// Specular:
                NdotL = max(0.0, NdotL);
                float node_10 = 0.0;
                float4 node_12 = tex2D(_Base,TRANSFORM_TEX(node_441.rg, _Base));
                float node_6 = 1.0;
                float node_5 = lerp( node_6, min(max((lerp(node_10,tex2D(_AOMapAlpha,TRANSFORM_TEX(node_441.rg, _AOMapAlpha)).a,_AOBurn)*_AOMultiplier),node_10),node_6), _AOActive );
                float3 specularColor = (lerp(node_10,lerp( node_12.a, tex2D(_SpecularCustomMap,TRANSFORM_TEX(node_441.rg, _SpecularCustomMap)).a, _CustomSpecular ),_SpecPower)*node_5*_SpecBurn*_SpecularColor.rgb);
                float3 specular = (floor(attenuation) * _LightColor0.xyz) * pow(max(0,dot(halfDirection,normalDirection)),specPow) * specularColor;
                float3 finalColor = 0;
                float3 diffuseLight = diffuse;
                float3 node_44 = ((_RimPower*pow(1.0-max(0,dot(normalDirection, viewDirection)),_RimFresnel))*_RimColor.rgb*tex2D(_RimMask,TRANSFORM_TEX(node_441.rg, _RimMask)).a);
                diffuseLight += node_44; // Diffuse Ambient Light
...
/////// Normals:
                ...
                float3 normalLocal = lerp(UnpackNormal(tex2D(_Normal,TRANSFORM_TEX(node_442.rg, _Normal))).rgb,float3(0,0,1),_NormalBurn);
                float3 normalDirection =  normalize(mul( normalLocal, tangentTransform )); // Perturbed normals
                ...
                clip(tex2D(_CutoutMask,TRANSFORM_TEX(node_442.rg, _CutoutMask)).a - 0.5);
...
///////// Gloss:
                float gloss = lerp( _Shininess, (tex2D(_Gloss,TRANSFORM_TEX(node_442.rg, _Gloss)).a*_Shininess), _UseGlossMap );
                float specPow = exp2( gloss * 10.0+1.0);
////// Specular:
                NdotL = max(0.0, NdotL);
                float node_10 = 0.0;
                float4 node_12 = tex2D(_Base,TRANSFORM_TEX(node_442.rg, _Base));
                float node_6 = 1.0;
                float node_5 = lerp( node_6, min(max((lerp(node_10,tex2D(_AOMapAlpha,TRANSFORM_TEX(node_442.rg, _AOMapAlpha)).a,_AOBurn)*_AOMultiplier),node_10),node_6), _AOActive );
                float3 specularColor = (lerp(node_10,lerp( node_12.a, tex2D(_SpecularCustomMap,TRANSFORM_TEX(node_442.rg, _SpecularCustomMap)).a, _CustomSpecular ),_SpecPower)*node_5*_SpecBurn*_SpecularColor.rgb);
...
/// Final Color:
        ...
        Pass {
            ...
            fixed4 frag(VertexOutput i) : COLOR {
                float2 node_443 = i.uv0;
                clip(tex2D(_CutoutMask,TRANSFORM_TEX(node_443.rg, _CutoutMask)).a - 0.5);
        ...
        Pass {
            ...
            fixed4 frag(VertexOutput i) : COLOR {
                float2 node_444 = i.uv0;
                clip(tex2D(_CutoutMask,TRANSFORM_TEX(node_444.rg, _CutoutMask)).a - 0.5);
