...
////// Emissive:
                float4 node_24 = tex2D(_GlossRRimGRefB,TRANSFORM_TEX(node_186.rg, _GlossRRimGRefB));
...
////// Specular:
                NdotL = max(0.0, NdotL);
                float node_62 = (tex2D(_SpecularMap,TRANSFORM_TEX(node_186.rg, _SpecularMap)).a*_SpecularLevel);
                float3 specularColor = float3(node_62,node_62,node_62);
                float3 specular = (floor(attenuation) * _LightColor0.xyz) * pow(max(0,dot(halfDirection,normalDirection)),specPow) * specularColor;
                float3 finalColor = 0;
                float3 diffuseLight = diffuse;
                finalColor += diffuseLight * tex2D(_Base,TRANSFORM_TEX(node_186.rg, _Base)).rgb;
                finalColor += specular;
                finalColor += emissive;
/// Final Color:
        ...
        Pass {
            ...
            fixed4 frag(VertexOutput i) : COLOR {
                ...
                float3 viewDirection = normalize(_WorldSpaceCameraPos.xyz - i.posWorld.xyz);
/////// Normals:
                float2 node_187 = i.uv0;
                float3 normalLocal = UnpackNormal(tex2D(_NormalMap,TRANSFORM_TEX(node_187.rg, _NormalMap))).rgb;
                float3 normalDirection =  normalize(mul( normalLocal, tangentTransform )); // Perturbed normals
                float3 viewReflectDirection = reflect( -viewDirection, normalDirection );
...
///////// Gloss:
                float4 node_24 = tex2D(_GlossRRimGRefB,TRANSFORM_TEX(node_187.rg, _GlossRRimGRefB));
                float gloss = (node_24.r*_GlossLevel);
                float specPow = exp2( gloss * 10.0+1.0);
////// Specular:
                NdotL = max(0.0, NdotL);
                float node_62 = (tex2D(_SpecularMap,TRANSFORM_TEX(node_187.rg, _SpecularMap)).a*_SpecularLevel);
                float3 specularColor = float3(node_62,node_62,node_62);
                float3 specular = attenColor * pow(max(0,dot(halfDirection,normalDirection)),specPow) * specularColor;
                float3 finalColor = 0;
                float3 diffuseLight = diffuse;
                finalColor += diffuseLight * tex2D(_Base,TRANSFORM_TEX(node_187.rg, _Base)).rgb;
                finalColor += specular;
/// Final Color:
