...
/////// Normals:
                ...
                float3 normalLocal = lerp(UnpackNormal(tex2D(_Normal,TRANSFORM_TEX(node_463.rg, _Normal))).rgb,float3(0,0,1),_NormalBurn);
...
/////// Diffuse:
                ...
                float node_10 = 0.0;
                float _UseLightWrap_var = lerp( node_10, (tex2D(_WrapMask,TRANSFORM_TEX(node_463.rg, _WrapMask)).a*_LightWrap), _UseLightWrap );
...
////// Emissive:
                float3 emissive = (texCUBE(_Reflection,viewReflectDirection).rgb*(pow(1.0-max(0,dot(normalDirection, viewDirection)),_RefFresnelRim)*_RefFresnelPower)*tex2D(_ReflectionMask,TRANSFORM_TEX(node_463.rg, _ReflectionMask)).a*_ReflectionPower*_ReflectionColor.rgb);
///////// Gloss:
                float gloss = lerp( _Shininess, (tex2D(_Gloss,TRANSFORM_TEX(node_463.rg, _Gloss)).a*_Shininess), _UseGlossMap );
                float specPow = exp2( gloss * 10.0+1.0);
////// Specular:
                NdotL = max(0.0, NdotL);
                float4 node_12 = tex2D(_Base,TRANSFORM_TEX(node_463.rg, _Base));
                float node_6 = 1.0;
                float node_5 = lerp( node_6, min(max((lerp(node_10,tex2D(_AOMapAlpha,TRANSFORM_TEX(node_463.rg, _AOMapAlpha)).a,_AOBurn)*_AOMultiplier),node_10),node_6), _AOActive );
                float3 specularColor = (lerp(node_10,lerp( node_12.a, tex2D(_SpecularCustomMap,TRANSFORM_TEX(node_463.rg, _SpecularCustomMap)).a, _CustomSpecular ),_SpecPower)*node_5*_SpecBurn*_SpecularColor.rgb);
                float3 specular = (floor(attenuation) * _LightColor0.xyz) * pow(max(0,dot(halfDirection,normalDirection)),specPow) * specularColor;
                float3 finalColor = 0;
                float3 diffuseLight = diffuse;
                diffuseLight += (lerp( UNITY_LIGHTMODEL_AMBIENT.rgb, ((_RimPower*pow(1.0-max(0,dot(normalDirection, viewDirection)),_RimFresnel))*_RimColor.rgb*tex2D(_RimMask,TRANSFORM_TEX(node_463.rg, _RimMask)).a), _CustomAmbient )*_AmbientPower*node_5); // Diffuse Ambient Light
...
/// Final Color:
        ...
        Pass {
            ...
            fixed4 frag(VertexOutput i) : COLOR {
                ...
                float3 viewDirection = normalize(_WorldSpaceCameraPos.xyz - i.posWorld.xyz);
/////// Normals:
                float2 node_464 = i.uv0;
                float3 normalLocal = lerp(UnpackNormal(tex2D(_Normal,TRANSFORM_TEX(node_464.rg, _Normal))).rgb,float3(0,0,1),_NormalBurn);
                float3 normalDirection =  normalize(mul( normalLocal, tangentTransform )); // Perturbed normals
                float3 viewReflectDirection = reflect( -viewDirection, normalDirection );
...
/////// Diffuse:
                float NdotL = dot( normalDirection, lightDirection );
                float node_10 = 0.0;
                float _UseLightWrap_var = lerp( node_10, (tex2D(_WrapMask,TRANSFORM_TEX(node_464.rg, _WrapMask)).a*_LightWrap), _UseLightWrap );
                float3 w = float3(_UseLightWrap_var,_UseLightWrap_var,_UseLightWrap_var)*0.5; // Light wrapping
                float3 NdotLWrap = NdotL * ( 1.0 - w );
                float3 forwardLight = max(float3(0.0,0.0,0.0), NdotLWrap + w );
                float3 diffuse = forwardLight * attenColor;
///////// Gloss:
                float gloss = lerp( _Shininess, (tex2D(_Gloss,TRANSFORM_TEX(node_464.rg, _Gloss)).a*_Shininess), _UseGlossMap );
                float specPow = exp2( gloss * 10.0+1.0);
////// Specular:
                NdotL = max(0.0, NdotL);
                float4 node_12 = tex2D(_Base,TRANSFORM_TEX(node_464.rg, _Base));
                float node_6 = 1.0;
                float node_5 = lerp( node_6, min(max((lerp(node_10,tex2D(_AOMapAlpha,TRANSFORM_TEX(node_464.rg, _AOMapAlpha)).a,_AOBurn)*_AOMultiplier),node_10),node_6), _AOActive );
                float3 specularColor = (lerp(node_10,lerp( node_12.a, tex2D(_SpecularCustomMap,TRANSFORM_TEX(node_464.rg, _SpecularCustomMap)).a, _CustomSpecular ),_SpecPower)*node_5*_SpecBurn*_SpecularColor.rgb);
